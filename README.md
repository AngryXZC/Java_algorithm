...
3. 双路快速排序
   1. 对于有大量重复元素的数组，双路快速排序的时间复杂度是O(nlogn)
   2. 双路快速排序的partition过程是将数组分为三部分：小于v，等于v，大于v
4. 三路快速排序
   1. 对于有大量重复元素的数组，三路快速排序在所有元素都相同的情况下的时间复杂度是O(n)
   2. 三路快速排序的partition过程是将数组分为四部分：小于v，等于v，大于v，待处理
...
5. 所以时间复杂度是O(nlogn)
#### 普通算法
分析复杂度的时候，通常使用最坏情况的复杂度，能找到一组数据100%的复杂度
#### 随机算法
