...
public class Section1_1 {
   ...
   //1.1.34　过滤。以下哪些任务需要（在数组中，比如）保存标准输入中的所有值？哪些可以被实现为一个过滤器且仅使用固定数量的变量和固定大小的数组（和 N 无关）？在每个问题中，输入都来自于标准输入且含有 N 个 0 到 1 的实数。
    ...
    //以下是对每个任务的分析：
    //
    //1. 打印出最大和最小的数：不需要保存所有的输入值，只需要两个变量来分别保存当前的最大值和最小值即可。
    //
    //2. 打印出所有数的中位数：需要保存所有的输入值，因为中位数的计算需要知道所有的数值。
    //
    //3. 打印出第 k 小的数，k 小于 100：需要保存所有的输入值，因为需要对所有的数进行排序才能找到第 k 小的数。
    //
    //4. 打印出所有数的平方和：不需要保存所有的输入值，只需要一个变量来保存当前的平方和即可。
    //
    //5. 打印出 N 个数的平均值：不需要保存所有的输入值，只需要两个变量来分别保存当前的总和和数量即可。
    //
    //6. 打印出大于平均值的数的百分比：需要保存所有的输入值，因为需要先计算出平均值，然后才能确定哪些数大于平均值。
    //
    //7. 将 N 个数按照升序打印：需要保存所有的输入值，因为需要对所有的数进行排序。
    //
    //8. 将 N 个数按照随机顺序打印：需要保存所有的输入值，因为需要对所有的数进行随机排序。
}
